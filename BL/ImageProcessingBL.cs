...
namespace BL
{
    public class ImageProcessingBL
    {
        /// <summary>
        ...
        public async void CharacterRecognition(Bitmap img)
        {
        ...
        public void FinalDefinition(List<Bitmap> parts, int heigth, int width, string resName)
        {
            new Task(() => {
        ...
        /// <returns></returns>
        public List<Bitmap> ImageDivision(Bitmap image)
        {
            Bitmap imgCopy = (Bitmap)image.Clone();
            List<AmountAndType> columns = Binarization(FindColorComponentOfTheImage(image, image.Width, image.Height), image.Width, image.Height, image);
        ...
        /// <param name="h"></param>
        /// <returns></returns>
        public byte FindColorComponentOfTheImage(Bitmap image, int w, int h)
        {
            uint white = 0;
        ...
        public List<AmountAndType> Binarization(byte component, int width,
        ...
        private void SecondColumnsCut(uint genStrOffset, List<AmountAndType> columns)
        {
            int i = 0;
            if (columns[i].type != 0)
                i++;
        ...
        /// </summary>
        private void AdditionalColumnsCheck(ref List<AmountAndType> columns)
        {
            uint sCounter = 0;
            uint nCounter = 0;
            uint bCounter = 0;
        ...
        /// <param name="heigth"> Высота картинки.</param>
        /// <param name="width"> Ширина картинки. </param>
        public List<AmountAndType> DivisionByColumns(int heigth, int width, Bitmap image)
        {
            List<AmountAndType> columns = new List<AmountAndType>();
            bool isBlack = false;
            bool previsBlack = false;
